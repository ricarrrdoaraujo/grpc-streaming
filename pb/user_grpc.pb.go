...
func (c *userServiceClient) AddUserStreamBi(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUserStreamBiClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUserStreamBi", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUserStreamBiClient{stream}
	return x, nil
}

type UserService_AddUserStreamBiClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUserStreamBiClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUserStreamBiClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUserStreamBiClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type UserServiceServer interface {
	...
	AddUserStreamBi(UserService_AddUserStreamBiServer) error
...
func (UnimplementedUserServiceServer) AddUserStreamBi(UserService_AddUserStreamBiServer) error {
	return status.Errorf(codes.Unimplemented, "method AddUserStreamBi not implemented")
}
...
func _UserService_AddUserStreamBi_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUserStreamBi(&userServiceAddUserStreamBiServer{stream})
}

type UserService_AddUserStreamBiServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUserStreamBiServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUserStreamBiServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUserStreamBiServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "AddUserStreamBi",
			Handler:       _UserService_AddUserStreamBi_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
